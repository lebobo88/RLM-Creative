...
- **Sub-crew (Helios dispatches only)**: video-synth, audio-foley, music-score, dialogue-mix
- **Continuous gate**: governance-c2pa watches every render via `post-render-validate` hook

## Mode
PARALLEL with budget-cap interrupts. Each completed asset publishes to `RLM/output/assets/` and increments a running cost ledger. Helios MUST pause if the ledger exceeds 90% of `brief.budget.production_usd` and either request HITL or trim.
...
- **video-synth**: receives motion jobs. Selects model (Kling / Veo / Seedance / Wan) per the `comfyui-workflow-recipes` skill. Writes outputs to `RLM/output/assets/video/`.
...
4. **Cost ledger**: append `{shot_id, engine, cost_usd, ts}` to `RLM/output/assets/_ledger.jsonl`.
...
## Output
- `RLM/output/assets/{type}/{shot_id}.{ext}` — every render (gated by hook)
- `RLM/output/copy/{brief_id}-canonical.md` (Erato) + `RLM/output/copy/{brief_id}-{channel}.md` (each head)
- `RLM/output/assets/_ledger.jsonl` — cost ledger
- `eights.memory.remember` per asset: `{type:"render", shot_id, cost_usd, model_type, outcome}`

## Handoff
- Phase 07 (QC + Clearance) consumes everything in `RLM/output/assets/` and `RLM/output/copy/`.
